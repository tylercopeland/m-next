...
const defaultStore = {
  ...
  screenLayout: {
    baseModel: 'Contacts',
    controls: {
      control1: { id: 'control1', name: 'IsTaxable', isBound: false },
    },
    fields: [
    ...
    <div data-testid="action-list-section">
      ...
      <ul>
        {values.map((value, index) => (
          ...
          <li key={index}>{value[valueKey]}</li>
        ))}
      </ul>
    </div>
  );
...
describe('Checkbox Editor', () => {
  ...
  const renderComponent = () => render(
    <Provider store={store}>
      <CheckboxBlockEditor {...props} />
    </Provider>
  );

  it('should render without crashing', () => {
    renderComponent();
    expect(screen.getByText('Edit the base configuration and styles of the checkbox.')).toBeInTheDocument();
  });
  ...
  it('should disable field dropdown when no available fields', () => {
    store = mockStore({
      ...
      screenLayout: {
        ...
        controls: {
          control1: { id: 'control1', name: 'IsActive', isBound: true },
          control2: { id: 'control2', name: 'IsTaxable', isBound: true },
        },
      },
    });
