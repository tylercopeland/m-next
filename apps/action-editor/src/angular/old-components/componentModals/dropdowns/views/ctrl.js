const ComponentDropdownViewsCtrl = function ($rootScope, $scope, $http, $filter, $timeout, ComponentModalSvc) {
    console.debug("ComponentDropdownViewsCtrl", $scope.control);
    $scope.svgIconStyle = {
    ...
    ComponentModalSvc
        ...
        .then(function (resp) {
            ...
            $rootScope.$broadcast("micb:refresh", {
                whereClause: $scope.filterDef[0].expression
            });
        });
...
ComponentDropdownViewsCtrl.$inject = ['$rootScope', '$scope', '$http', '$filter', '$timeout', 'ComponentModalSvc'];
export default ComponentDropdownViewsCtrl;