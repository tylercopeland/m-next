  ...
  function FoundationMQInit(helpers, foundationApi, u){
    ...
    function init() {
      ...
      for(var key in mediaQueries) {
        var replacedValue = '';
        if (mediaQueries[key]) {
          mediaQueries[key].replace('rem', 'em');
        }
        
        mediaQueries[key] = 'only screen and (min-width: ' + replacedValue + ')';
      }
    ...
    function compile(tElement, tAttrs, transclude) {
      ...
      function postLink(scope, element, attrs) {
        ...
        element.on('click', function(e) {
          ...
          if(!matchMedia(globalQueries.medium).matches && srcEl.formAction && srcEl.formAction.length > 0) {
