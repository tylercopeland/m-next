...
const propTypes = {
  ...
  rowBeingDragged: PropTypes.bool,
...
function DraggableRow({
  ...
  const [isDragVisible, setIsDragVisible] = useState(alwaysShowDragHandles);
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const isEditing = useMemo(() => editingRow === index, [editingRow, index]);
  const isReadOnly = useMemo(() => !editable, [editable]);
  const firstVisibleIndex = useMemo(() => columns.findIndex((x) => x.visible), [columns]);
  ...
  const rowMouseEnter = () => {
    if (showDragOnHover && !alwaysShowDragHandles && !rowBeingDragged) setIsDragVisible(true);
  };
  const rowMouseLeave = () => {
    if (showDragOnHover && !alwaysShowDragHandles) setIsDragVisible(false);
  };

  const mouseEnter = () => {
    setIsDragDisabled(false);
    if (!alwaysShowDragHandles && !rowBeingDragged) setIsDragVisible(true);
  };
  const mouseLeave = () => {
    setIsDragDisabled(true);
    if (!alwaysShowDragHandles && !showDragOnHover) setIsDragVisible(false);
  };
    ...
    <Draggable key={id} draggableId={id} index={index} isDragDisabled={isDragDisabled}>
      {(providedInner, snapshotInner) => (
        ...
        >
            ...
            >
              {(primaryKey || status !== STATUSES.blank) && status !== STATUSES.deleted && (
                <s.CheckContainer>
                  ...
                  {!isDragVisible && <span style={{ width: 16 }} />}
                  <Checkbox
          ...
          {columns.map((col, cellIdx) => {
            ...
            return (
              <Cell
                ...
                isFirstVisible={firstVisibleIndex === cellIdx && !showMobileCardColumn}
          ...
          {canDelete && editable && !snapshotInner.isDragging && (
            <s.ActionCell id={`${id}-LAST`} className='hideOutline' draggable>
